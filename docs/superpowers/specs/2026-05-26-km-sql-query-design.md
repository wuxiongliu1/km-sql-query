...
└── app               # 最终可执行 Spring Boot 应用
```

**依赖关系**：`app → starter → (admin, api) → core`
...
- 片段复用：`<sql>` 和 `<include>`
- 参数占位符：`#{xxx}`（预编译绑定）；`${xxx}` 默认禁用，可通过服务端配置显式开启
- 模板必须以 `<select>/<insert>/<update>/<delete>` 标签开头
- 标签类型必须与最终 SQL 首关键字一致，且单个模板只允许一条语句
- SELECT 支持 `page`、`size`，未传时最多返回 `km.sql-query.max-rows` 行
...
1. **管理端保存时自动触发**：`admin` 模块在 SqlConfig JPA save/delete 操作后清理模板解析缓存，下次请求按新的 `SqlSource` 替换 MappedStatement
...
- 支持保存前临时连接测试，以及已保存数据源的连通性和连接池状态检查
- 删除数据源前检查 SQL 配置引用，存在引用时拒绝删除
...
GET    /api/admin/sqlConfig/list         # 分页列表，支持 sqlPath/datasourceId/folder 筛选
GET    /api/admin/sqlConfig/folders      # folder 去重列表
POST   /api/admin/sqlConfig/test         # 测试未发布模板，全程回滚
...
POST   /api/admin/datasource/test        # 保存前连接测试
GET    /api/admin/datasource/{id}/health # 连通性与连接池状态
```

密码 AES 加密存储，库中不存明文，管理接口响应不返回密码字段。生产环境通过 `KM_SQL_QUERY_AES_KEY` 配置 16、24 或 32 字节密钥。

## 安全与异常处理

- SQL 注入防护：`#{}` 走 PreparedStatement 预编译；`${}` 默认禁止，多语句与类型伪装均拒绝执行
- 连接泄漏：HikariCP 自带超时回收
- DML 防护：保存时及运行时双重检查，最终生成的 UPDATE/DELETE 必须包含真实 WHERE 条件
- 执行超时：通过 `km.sql-query.timeout-seconds` 配置，默认 30 秒
...
- `km.sql.query.duration`：按 `sqlPath`、`outcome` 聚合接口耗时与成功/失败次数
- `km.sql.query.returned.rows`：按 `sqlPath` 统计返回或影响行数
- 超过 `km.sql-query.slow-sql-threshold-ms` 的调用输出慢 SQL 警告日志
- Actuator 暴露 `/actuator/health`、`/actuator/metrics` 与 `/actuator/prometheus`
